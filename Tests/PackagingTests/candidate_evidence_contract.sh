...
mkdir -p "$REPO" "$RELEASE/Patchwright.app/Contents/MacOS" "$RELEASE/Patchwright.app/Contents/Helpers" "$EVIDENCE"
...
jq -n '{}' >"$EVIDENCE/build-metadata.json"
jq -n '{schema_version:1,clean:true,findings:[]}' >"$EVIDENCE/secret-scan.json"
jq -n '{schema_version:1,links:[]}' >"$EVIDENCE/SYMLINKS.json"
jq -n '{schema_version:1,kind:"app",status:"Accepted",request_id:"app-id",stapled:true,stapler_validated:true}' >"$EVIDENCE/notary-app.json"
jq -n --arg digest "$DIGEST" '{schema_version:1,kind:"dmg",status:"Accepted",request_id:"dmg-id",stapled:true,stapler_validated:true,final_sha256:$digest}' >"$EVIDENCE/notary-dmg.json"
...
for gate in repo.json secret-scan-gate.json compliance-gate.json; do
  jq -e --arg digest "$DIGEST" '.status == "pass" and .artifact_sha256 == $digest' "$EVIDENCE/$gate" >/dev/null
done
