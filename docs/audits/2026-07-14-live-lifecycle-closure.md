...
- GitHub writes cover branch creation/push, comments, reviews, checks, draft PRs, update branch, ready-for-review, issue/PR closure, merge-queue handoff, and exact-SHA merge. Every write requires a fresh typed preview, matching approval, precondition validation, and idempotency claim.
...
## Historical audit-record caveat

The two sandbox tasks visible in the current local database were created and merged by the pre-fix engine. Their immutable remote evidence is correct, but their stored state remains `Awaiting Delivery Approval`; the new atomic completion path intentionally does not guess or rewrite historical task records. New successful merge executions use the corrected transaction and appear under Completed. The live sandbox PR and issue themselves are already merged/closed.

## Remaining boundaries

- GitHub review submission is implemented. Resolving individual GitHub review threads is not yet a first-class typed action because the current ingestion model does not retain GraphQL review-thread node IDs.
- The app is locally ad-hoc signed. This machine has no installed Developer ID Application identity or notary profile, so Developer ID signing, hardened-runtime distribution proof, notarization/stapling, Gatekeeper acceptance, DMG qualification, and clean-machine validation remain `blocked:external`.
