...
The webhook relay verifies `X-Hub-Signature-256` against the raw body before typed parsing, rejects bodies larger than 1 MiB, validates a supported GitHub event/action and its minimum typed identity, and atomically commits only a sanitized identity envelope plus the raw-body digest to an owner-only SQLite inbox before returning `202 Accepted`. Duplicate `X-GitHub-Delivery` values remain duplicates after restart. A bounded outbox forwards accepted envelopes to `PATCHWRIGHT_ENGINE_SOCKET` over an owner-only Unix socket with bounded frames and timeouts; outages use durable bounded backoff. The engine atomically deduplicates delivery IDs and wakes only pending monitors matching the exact repository and, when present, pull-request number. Set `PATCHWRIGHT_RELAY_DATABASE` (or `--database`) to an owner-only regular file in an owner-only directory. The relay rejects non-loopback bind addresses; terminate authenticated HTTPS or a tunnel locally and use the required `serve` subcommand. Pass `PATCHWRIGHT_GITHUB_WEBHOOK_SECRET_FILE` as an absolute path to a regular, non-symlink secret file owned by the operator with mode `0400` or `0600`; never put the raw secret in an environment variable, command line, repository file, or log. Supply GitHub App private keys and installation identifiers only through the documented protected reference and runtime configuration boundaries.

GitHub does not automatically redeliver a failed webhook after the relay
returns a non-2xx response before durable acceptance. An authorized GitHub App owner must request redelivery from the App's
Recent deliveries page or through GitHub's authenticated redelivery API. Treat
that as an operator action. Deliveries already accepted with `202` are retried
locally after relay or engine restart and do not need GitHub redelivery.
