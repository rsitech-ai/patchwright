...
def build_documents(
    cargo: dict[str, Any], swift: dict[str, Any], components: list[tuple[str, Path]]
    ...
    notice_lines = [
        ...
        "",
        "| Package | Version | Declared license | Ecosystem | Source |",
        "| --- | --- | --- | --- | --- |",
    ]
    for item in sorted(notices, key=lambda row: (row["name"].casefold(), row["version"], row["ecosystem"])):
        cells = [item[key].replace("|", "\\|") for key in ("name", "version", "license", "ecosystem", "source")]
...
def main() -> int:
    args = parse_args()
    try:
        document, notices = build_documents(
