...
#[tokio::test]
async fn task_plan_accepts_a_pristine_no_checkout_managed_clone() {
    let directory = tempfile::tempdir().unwrap();
    std::fs::set_permissions(directory.path(), std::fs::Permissions::from_mode(0o700)).unwrap();
    ...
    .unwrap();
    git(&origin, &["add", "Cargo.toml", "Cargo.lock"]);
    git(
        &origin,
        &[
            "-c",
    ...
    assert!(
        !git(&repository, &["status", "--porcelain=v1"]).is_empty(),
        "fixture must reproduce Git's no-checkout dirty status"
    );
    ...
    let created = call(
        &mut stream,
        json!({"jsonrpc":"2.0","id":2,"method":"task.createFromGitHub","params":{
            "repositoryFullName":"acme/widget","itemNumber":"7",
            "expectedUpdatedAt":"2026-07-13T12:00:00Z"
        }}),
    )
...
async fn rpc_plans_and_prepares_an_isolated_worktree_before_codex() {
    ...
    let ready = call(
        &mut stream,
        json!({"jsonrpc":"2.0","id":91,"method":"task.readyForDelivery","params":{"taskId":task_id}}),
    )
    .await;
    assert!(ready.get("error").is_none(), "verification failed: {ready}");
    assert_eq!(ready["result"]["state"], "awaitingDeliveryApproval");
