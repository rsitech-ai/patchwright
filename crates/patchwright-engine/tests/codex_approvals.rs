...
use patchwright_engine::codex::process::{CodexExecutable, CodexProcessConfig, CodexProcessFactory};
use patchwright_engine::codex::service::{CodexApprovalKind, CodexApprovalState, CodexService, CodexServiceError};
use std::sync::Mutex;
use tempfile::tempdir;

fn advance_to_preparing(task: &mut Task) {
    for state in [TaskState::Assessing, TaskState::Planned, TaskState::AwaitingPreparationApproval, TaskState::Preparing] {
...
async fn harness() -> (tempfile::TempDir, Mutex<EventStore>, CodexService, CodexProcessFactory, String, Task) {
    ...
    (root, store, CodexService::new(factory.clone(), version.clone()), factory, version, task)
}
...
async fn command_and_file_requests_are_exact_codex_runtime_approvals_only() {
    ...
    service.start_turn(task.id, "message-1", "Implement.", &store).await.unwrap();
    let approvals = service.approvals(task.id, &store).await.unwrap();
    assert_eq!(approvals.len(), 2);
    assert!(approvals.iter().all(|value| value.class == ApprovalClass::CodexRuntime));
    assert_eq!(approvals[0].process_generation, service.status(task.id, &store).unwrap().process_generation.unwrap());
    assert_eq!(approvals.iter().map(|value| value.kind).collect::<Vec<_>>(), [CodexApprovalKind::Command, CodexApprovalKind::FileChange]);
    assert!(approvals.iter().all(|value| value.thread_id == "thread-approval" && value.turn_id == "turn-approval"));
    assert!(approvals.iter().all(|value| value.state == CodexApprovalState::Pending));

    let generation = approvals[0].process_generation;
    let approved = service.resolve_approval(task.id, approvals[0].id, generation, true, &store).await.unwrap();
    assert_eq!(approved.state, CodexApprovalState::Approved);
    let duplicate = service.resolve_approval(task.id, approvals[0].id, generation, false, &store).await.unwrap();
    assert_eq!(duplicate.state, CodexApprovalState::Approved, "a duplicate decision must be idempotent");
    let declined = service.resolve_approval(task.id, approvals[1].id, generation, false, &store).await.unwrap();
...
async fn expired_and_wrong_generation_approvals_fail_closed_and_survive_restart() {
    ...
    store.lock().unwrap().save_codex_runtime_approval(&expired).unwrap();
    assert!(matches!(service.resolve_approval(task.id, expired.id, expired.process_generation, true, &store).await, Err(CodexServiceError::ApprovalInvalid)));
    assert_eq!(store.lock().unwrap().codex_runtime_approval(expired.id).unwrap().unwrap().state, CodexApprovalState::Expired);
    ...
    assert_eq!(reopened.lock().unwrap().codex_runtime_approval(pending.id).unwrap().unwrap(), pending);
    let mut restarted = CodexService::new(factory, version);
    let status = restarted.start(task.id, &reopened).await.unwrap();
    assert_ne!(status.process_generation.unwrap(), pending.process_generation);
    assert!(matches!(restarted.resolve_approval(task.id, pending.id, pending.process_generation, true, &reopened).await, Err(CodexServiceError::ApprovalInvalid)));
    assert_eq!(reopened.lock().unwrap().codex_runtime_approval(pending.id).unwrap().unwrap().state, CodexApprovalState::Invalidated);
}
