...
fn advance_to_preparing(task: &mut Task) {
    for state in [TaskState::Assessing, TaskState::Planned, TaskState::AwaitingPreparationApproval, TaskState::Preparing] { task.transition(state).unwrap(); }
}
...
async fn service(root: &std::path::Path, body: &str, grace: std::time::Duration) -> CodexService {
    ...
    let config = CodexProcessConfig { shutdown_grace: grace, ..CodexProcessConfig::default() };
...
async fn cancel_before_turn_retains_worktree_and_unrelated_process() {
    ...
    assert_eq!(store.lock().unwrap().load_task(task.id).unwrap().unwrap().state, TaskState::Cancelled);
    assert_eq!(std::fs::read_to_string(worktree.join("evidence.txt")).unwrap(), "retain");
    assert!(unrelated.try_wait().unwrap().is_none(), "unrelated process must survive");
...
printf '%s\n' "$interrupt" >> '{}'
while :; do sleep 1; done"#, handshake(), interrupt_log.display());
    let mut service = service(root.path(), &body, std::time::Duration::from_millis(100)).await;
    let worktree = root.path().join("worktree");
    std::fs::create_dir(&worktree).unwrap();
    ...
    store.lock().unwrap().save_task(&task, "prepared").unwrap();
    service.start(task.id, &store).await.unwrap();
    service.start_turn(task.id, "message-1", "Work until interrupted.", &store).await.unwrap();
    service.interrupt(task.id, false, &store).await.unwrap();
    let lines = std::fs::read_to_string(&interrupt_log).unwrap();
    assert_eq!(lines.lines().count(), 1, "turn/interrupt must be sent exactly once");
    assert!(lines.contains("turn/interrupt"));
    assert_eq!(store.lock().unwrap().load_task(task.id).unwrap().unwrap().state, TaskState::Paused);
    assert!(service.start_turn(task.id, "message-after-pause", "Must not run.", &store).await.is_err());
    let resumed = service.start(task.id, &store).await.unwrap();
    assert!(resumed.can_send);
    assert_eq!(store.lock().unwrap().load_task(task.id).unwrap().unwrap().state, TaskState::Implementing);
}
...
while IFS= read -r line; do :; done"#, handshake());
    ...
    store.lock().unwrap().save_task(&task, "prepared").unwrap();
    service.start(task.id, &store).await.unwrap();
    service.start_turn(task.id, "message-1", "Finish now.", &store).await.unwrap();
    service.interrupt(task.id, true, &store).await.unwrap();
    assert_eq!(store.lock().unwrap().load_task(task.id).unwrap().unwrap().state, TaskState::Implementing);
    assert!(store.lock().unwrap().codex_events(task.id, 0).unwrap().iter().any(|event| event.kind == "turnCompleted"));
}
...
exit 70"#, handshake());
    ...
    store.lock().unwrap().save_task(&task, "prepared").unwrap();
    service.start(task.id, &store).await.unwrap();
    service.start_turn(task.id, "message-1", "Crash after accepting.", &store).await.unwrap();
    assert!(service.events(task.id, 0, 100, &store).await.is_err());
    assert_eq!(store.lock().unwrap().load_task(task.id).unwrap().unwrap().state, TaskState::Failed);
    assert_eq!(std::fs::read_to_string(worktree.join("partial.txt")).unwrap(), "retained");
    assert!(store.lock().unwrap().codex_events(task.id, 0).unwrap().iter().any(|event| event.summary.contains("exited unexpectedly")));
}
